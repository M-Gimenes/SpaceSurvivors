...
class Hud(pg.sprite.Sprite):
    def __init__(self) -> None:
        ...
        width_health = health_bars['left'].get_width() + health_bars['middle'].get_width() + health_bars['right'].get_width()
        height_health = health_bars['left'].get_height()
        
        union_health = pg.surface.Surface((width_health,height_health), pg.SRCALPHA)
        union_health.blits(blit_sequence=((health_bars['left'], (0,0)),
                                        (health_bars['middle'], (health_bars['left'].get_width(),0)),
        ...
        healthBar_pos = (20,20)
        healthBar = {'surface':healthBar_surf, 'mask': healthBar_mask, 'pos': healthBar_pos}
        ...
        width_stamina = stamina_bars['left'].get_width() + stamina_bars['middle'].get_width() + stamina_bars['right'].get_width()
        height_stamina = stamina_bars['left'].get_height()
        
        union_stamina = pg.surface.Surface((width_stamina,height_stamina), pg.SRCALPHA)
        union_stamina.blits(blit_sequence=((stamina_bars['left'], (0,0)),
                                        (stamina_bars['middle'], (stamina_bars['left'].get_width(),0)),
        ...
        staminaBar = {'surface':staminaBar_surf, 'mask': staminaBar_mask, 'pos': staminaBar_pos}
        ...
        width_dodge = dodge_bars['left'].get_width() + dodge_bars['middle'].get_width() + dodge_bars['right'].get_width()
        height_dodge = dodge_bars['left'].get_height()
        
        union_dodge = pg.surface.Surface((width_dodge, height_dodge), pg.SRCALPHA)
        union_dodge.blits(blit_sequence=((dodge_bars['left'], (0,0)),
                                        (dodge_bars['middle'], (dodge_bars['left'].get_width(),0)),
        ...
        dodgeBar_pos = (20,52)
        dodgeBar = {'surface':dodgeBar_surf, 'mask': dodgeBar_mask, 'pos': dodgeBar_pos}
        ...
        width_xp = xp_bars['left'].get_width() + xp_bars['middle'].get_width() + xp_bars['right'].get_width()
        ...
        union_xp.blits(blit_sequence=((xp_bars['left'], (0,0)),
                                        (xp_bars['middle'], (xp_bars['left'].get_width(),0)),
        ...
        self.bars = {'health':healthBar, 'stamina':staminaBar, 'dodge':dodgeBar, 'xp': xpBar}
    ...
    def draw_bar(self, max, current, bar):
        ...
        background = pg.surface.Surface((self.bars[bar]['surface'].get_size()), pg.SRCALPHA)
        current = pg.transform.scale_by(self.bars[bar]['surface'], (current/max, 1))
        background.blit(current, (0,0))
        if len(self.bars[bar]['mask'].outline()) > 2:
            pg.draw.polygon(background, (UI.colors['white']), self.bars[bar]['mask'].outline(), 2)
