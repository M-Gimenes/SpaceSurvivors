...
class Ship(pg.sprite.Sprite):
    ...
    def __init__(self, x, y) -> None:
        ...
        self.text = Button(UI.display_text('lock_ship_2'),
                           UI.fonts['sm'], x, y+50, 0, 'text')
    ...
    def block(self):
        if not self.available[self.current]:
            ...
            if len(self.mask.outline()) > 2:
                pg.draw.polygon(self.black, (0, 0, 0, 225),
                                self.mask.outline(), 0)
                pg.draw.polygon(
                    self.black, UI.colors['white'], self.mask.outline(), 1)
